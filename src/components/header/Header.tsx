    ...
    <div className={styles.header}>
      <div className={styles.header__container}>
        ...
        <div className={styles.header__search}>
          <div className={searchVisible ? styles.header__search__input : `${styles.header__search__input} ${styles.header__none}`} >
            <input onChange={handleChangeSearch} type="text" placeholder="введите название фильма или сериала" />
            {searchedList && 
            <ul ref={ref}>
            {searchedList?.map((item: any)=>{
              return(
                <NavLink className='link-class' to={`/movie/${item.id}`}>
                  <li className={styles.header__search__li}  onClick={()=> setSearch(undefined)}>
                    <div>
                      <span className={styles.header__search__li__name}>{item.name}</span>
                      <span className={styles.header__search__li__year}>{`(${item.year})`}</span>
                    </div>
                    <span className={styles.header__search__li__rating}>{item.rating.kp !== 0 ? `${item.rating.kp.toFixed(1)}` : '?'}</span>
                  </li>
                </NavLink>
              )
            })}
          ...
          <button className={styles.header__search__btn}  onClick={handleSearchVisible}>
